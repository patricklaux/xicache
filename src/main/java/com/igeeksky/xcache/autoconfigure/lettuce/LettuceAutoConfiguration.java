...
class LettuceAutoConfiguration {
    ...
    private static void setClientOptionsBuilder(
        ...
        if (clientOptions != null) {
            ...
            Boolean cancelCommandsOnReconnectFailure = clientOptions.getCancelCommandsOnReconnectFailure();
            if (cancelCommandsOnReconnectFailure != null) {
                builder.cancelCommandsOnReconnectFailure(cancelCommandsOnReconnectFailure);
            }
    ...
    private static void setTimeoutOptionsBuilder(Lettuce.TimeoutOptions options, TimeoutOptions.Builder builder) {
        ...
        Boolean timeoutCommands = options.getTimeoutCommands();
        if (timeoutCommands != null) {
            builder.timeoutCommands(timeoutCommands);
        }
    ...
    private static void setSocketOptionsBuilder(Lettuce.SocketOptions options, SocketOptions.Builder builder) {
        if (options == null) {
            return;
        }
        ...
        if (keepAlive != null) {
            ...
            if (count != null) {
                keepAliveBuilder.count(count);
            }

            Boolean enabled = keepAlive.getEnabled();
            if (enabled != null) {
                keepAliveBuilder.enable(enabled);
            }

            Long idle = keepAlive.getIdle();
            if (idle != null) {
                keepAliveBuilder.idle(Duration.ofMillis(idle));
            }

            Long interval = keepAlive.getInterval();
            if (interval != null) {
                keepAliveBuilder.interval(Duration.ofMillis(interval));
            }

            builder.keepAlive(keepAliveBuilder.build());
        }
    }

    private static void setSslOptionsBuilder(Lettuce.SslOptions options, SslOptions.Builder builder) {
        if (options == null) {
            return;
        }
        ...
        if (StringUtils.hasLength(keystore)) {
            try {
                ...
                if (StringUtils.hasLength(keystorePassword)) {
                    builder.keystore(url, keystorePassword.toCharArray());
                } else {
                    builder.keystore(url);
                }
        ...
        if (StringUtils.hasLength(truststore)) {
            ...
            try {
                ...
                if (StringUtils.hasLength(truststorePassword)) {
                    builder.truststore(url, truststorePassword);
                } else {
                    builder.truststore(url);
                }
            } catch (FileNotFoundException e) {
                throw new RuntimeException(e);
            }
        ...
        if (CollectionUtils.isNotEmpty(cipherSuites)) {
            builder.cipherSuites(cipherSuites.toArray(new String[0]));
        }

        Long handshakeTimeout = options.getHandshakeTimeout();
        if (handshakeTimeout != null) {
            builder.handshakeTimeout(Duration.ofMillis(handshakeTimeout));
        }
    }
